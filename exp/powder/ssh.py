...
class SSHConnection:
    ...
    def __init__(self, ip_address, username=None, password=None, prompt=DEFAULT_PROMPT):
        ...
        if username is None:
            try:
                ...
                logging.error(
                    "no USER variable in environment variable and no username provided"
                )
                raise ValueError
    ...
    def open(self):
        ...
        while retry_count < 4:
            ...
            self.sshresponse = self.ssh.expect(
                [
                    self.prompt,
            ...
            if self.sshresponse == 0:
                ...
                if self.sshresponse == 0:
                    ...
                    if self.password is None:
                        logging.error(
                            "failed to login --- ssh key is encrypted but no pword provided"
                        )
                        raise ValueError
                    ...
                    if self.sshresponse == 0:
                        ...
                        logging.debug(
                            "failed to login --- response: {}".format(self.sshresponse)
                        )
                ...
                self.command(
                    'ifconfig | egrep --color=never "inet addr:|inet "', self.prompt
                )
                self.sshresponse = self.ssh.expect(
    ...
    def command(self, commandline, expectedline=DEFAULT_PROMPT, timeout=5):
        ...
        self.sshresponse = self.ssh.expect(
            [expectedline, pexpect.EOF, pexpect.TIMEOUT], timeout=timeout
        )
        if self.sshresponse == 0:
            pass
        elif self.sshresponse == 1:
    ...
    def copy_from(self, remote_path, local_path):
        cmd = "scp {}@{}:{} {}".format(
            self.username, self.ip_address, remote_path, local_path
        )
        return self.copy(cmd)

    def copy_to(self, local_path, remote_path):
        cmd = "scp {} {}@{}:{}".format(
            local_path, self.username, self.ip_address, remote_path
        )
        return self.copy(cmd)
        ...
        while retry_count < 10:
            ...
            if scp_response == 0:
                ...
                if scp_response == 0:
                    ...
                    if self.password is None:
                        logging.error(
                            "failed to scp --- ssh key is encrypted but no pword provided"
                        )
                        raise ValueError
                    ...
                    if scp_response == 0:
                        ...
                        logging.debug(
                            "failed to scp --- response: {}".format(scp_response)
                        )
                        logging.debug("retry count: {}".format(retry_count))
                    return scp_response
                else:
                    logging.debug(
                        "scp failed with scp response {}".format(scp_response)
                    )
                    logging.debug("retry count: {}".format(retry_count))
            elif scp_response == 1:
                if self.password is None:
