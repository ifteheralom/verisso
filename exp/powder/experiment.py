...
class PowderExperiment:
    ...
    def __init__(self, experiment_name: str, project_name: str, profile_name: str):
        if len(experiment_name) > 16:
            logging.error(
                "Experiment name {} is too long (cannot exceed {} characters)".format(
                    experiment_name, self.MAX_NAME_LENGTH
                )
            )
            sys.exit(1)
        ...
        logging.info(
            "initialized experiment {} based on profile {} under project {}".format(
                experiment_name, profile_name, project_name
            )
        )
    ...
    def start_and_wait(self):
        ...
        rval, response = prpc.start_experiment(
            self.experiment_name, self.project_name, self.profile_name
        )
    ...
    def terminate(self):
        ...
        rval, response = prpc.terminate_experiment(
    ...
    def _get_manifests(self):
        ...
        if rval == prpc.RESPONSE_SUCCESS:
            ...
            self._manifests = [
                xmltodict.parse(response_json[key]) for key in response_json.keys()
            ]
            ...
            for node in nodes:
                ...
                try:
                    ...
                    self.nodes[client_id] = Node(
                        client_id=client_id, ip_address=ipv4, hostname=hostname
                    )
    ...
    def _get_status(self):
        ...
        """
        rval, response = prpc.get_experiment_status(
            self.project_name, self.experiment_name
        )
        if rval == prpc.RESPONSE_SUCCESS:
            ...
            self.still_provisioning = self.status in [
                self.EXPERIMENT_PROVISIONING,
...
class Node:
    ...
    def __str__(self):
        return f"Node(client_id={self.client_id}, ip_address={self.ip_address}, hostname={self.hostname})"
